...
/* ─────────────────────────────────────────
   VIDEO DATA — exported so StudentSuccessPreview can import & pick randomly
───────────────────────────────────────── */
export const LOCAL_VIDEOS = [
  { id: 'local_1', type: 'local', ratio: '9/16', src: 'https://www.w3schools.com/html/mov_bbb.mp4',  name: 'Rahul Kumar', company: 'TCS',      pkg: '3.5 LPA', batch: '2024', bg: 'linear-gradient(160deg,#8B1A13,#C0261E 50%,#0A1628)' },
  { id: 'local_2', type: 'local', ratio: '9/16', src: 'https://www.w3schools.com/html/movie.mp4',   name: 'Priya Singh',  company: 'Infosys',  pkg: '4.0 LPA', batch: '2024', bg: 'linear-gradient(135deg,#1A56DB,#0A1628)' },
  { id: 'local_3', type: 'local', ratio: '9/16', src: 'https://www.w3schools.com/html/mov_bbb.mp4', name: 'Amit Sharma', company: 'Wipro',     pkg: '3.5 LPA', batch: '2023', bg: 'linear-gradient(135deg,#0B6E4F,#0A1628)' },
  { id: 'local_4', type: 'local', ratio: '9/16', src: 'https://www.w3schools.com/html/movie.mp4',   name: 'Neha Gupta',  company: 'Cognizant', pkg: '4.5 LPA', batch: '2024', bg: 'linear-gradient(135deg,#D4AF37,#8B1A13)' },
  { id: 'local_5', type: 'local', ratio: '9/16', src: 'https://www.w3schools.com/html/mov_bbb.mp4', name: 'Ravi Verma',  company: 'Accenture', pkg: '4.0 LPA', batch: '2023', bg: 'linear-gradient(135deg,#8B1A13,#1A56DB)' },
]

export const SHORT_VIDEOS = [
  { id: 'short_1', type: 'youtube_short', ratio: '9/16', embedId: 'dQw4w9WgXcQ', name: 'Success Story',  company: 'TCS',      pkg: 'Placed!', batch: '2024', bg: 'linear-gradient(160deg,#8B1A13,#0A1628)' },
  { id: 'short_2', type: 'youtube_short', ratio: '9/16', embedId: '9bZkp7q19f0', name: 'Campus Drive',   company: 'Infosys',  pkg: 'Placed!', batch: '2024', bg: 'linear-gradient(160deg,#1A56DB,#0A1628)' },
  { id: 'short_3', type: 'youtube_short', ratio: '9/16', embedId: 'dQw4w9WgXcQ', name: 'Placement Tips', company: 'Wipro',    pkg: 'Placed!', batch: '2024', bg: 'linear-gradient(160deg,#0B6E4F,#0A1628)' },
  { id: 'short_4', type: 'youtube_short', ratio: '9/16', embedId: '9bZkp7q19f0', name: 'Interview Exp',  company: 'HCL',      pkg: 'Placed!', batch: '2023', bg: 'linear-gradient(160deg,#D4AF37,#8B1A13)' },
  { id: 'short_5', type: 'youtube_short', ratio: '9/16', embedId: 'dQw4w9WgXcQ', name: 'Offer Letter',   company: 'Capgemini',pkg: 'Placed!', batch: '2024', bg: 'linear-gradient(160deg,#8B1A13,#1A56DB)' },
]

export const LONG_VIDEOS = [
  { id: 'long_1', type: 'youtube_long', ratio: '16/9', embedId: 'dQw4w9WgXcQ', name: 'Drive Highlights 2024',        company: 'Multiple',    pkg: 'Drive', batch: '2024', bg: 'linear-gradient(135deg,#0A1628,#1A56DB)', views: '12K',  duration: '18:42' },
  { id: 'long_2', type: 'youtube_long', ratio: '16/9', embedId: '9bZkp7q19f0', name: 'TCS Recruitment Full Process', company: 'TCS',         pkg: 'Drive', batch: '2024', bg: 'linear-gradient(135deg,#0A1628,#0B6E4F)', views: '8.4K', duration: '24:15' },
  { id: 'long_3', type: 'youtube_long', ratio: '16/9', embedId: 'dQw4w9WgXcQ', name: 'Crack Aptitude Tests',         company: 'Tips',        pkg: 'Guide', batch: '2024', bg: 'linear-gradient(135deg,#1A56DB,#8B1A13)', views: '21K',  duration: '31:08' },
  { id: 'long_4', type: 'youtube_long', ratio: '16/9', embedId: '9bZkp7q19f0', name: 'Mock Interview Session',       company: 'Expert',      pkg: 'Tips',  batch: '2023', bg: 'linear-gradient(135deg,#0A1628,#C0261E)', views: '6.2K', duration: '45:22' },
  { id: 'long_5', type: 'youtube_long', ratio: '16/9', embedId: 'dQw4w9WgXcQ', name: 'Resume Building Masterclass',  company: 'Career Tips', pkg: 'Guide', batch: '2024', bg: 'linear-gradient(135deg,#0B6E4F,#0A1628)', views: '15K',  duration: '27:55' },
  { id: 'long_6', type: 'youtube_long', ratio: '16/9', embedId: '9bZkp7q19f0', name: 'Infosys & Wipro Walkthrough',  company: 'Multiple',    pkg: 'Drive', batch: '2024', bg: 'linear-gradient(135deg,#D4AF37,#0A1628)', views: '9.8K', duration: '19:30' },
]

/* SHORT_EMBEDS — reshaped for the Shorts grid on this page only */
const SHORT_EMBEDS = SHORT_VIDEOS.map(v => ({
  type: 'youtube', id: v.embedId, label: v.name + ' — ' + v.company,
}))
...
function LocalVideoCard({ video }) {
  ...
  return (
    <div style={ls.card} onClick={toggle}>
      <video ref={videoRef} src={video.src} style={ls.video} loop playsInline onEnded={() => setPlaying(false)} />
...
const ls = {
  card: { position: 'relative', width: '100%', aspectRatio: '9/16', borderRadius: '16px', overflow: 'hidden', background: '#0A1628', cursor: 'pointer', boxShadow: '0 8px 32px rgba(0,0,0,0.25)', flexShrink: 0 },
  video: { position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' },
  overlay: { position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, transparent 40%, rgba(0,0,0,0.7))', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'opacity .25s' },
  pkgBadge: { position: 'absolute', top: '0.75rem', left: '0.75rem', background: '#D4AF37', color: '#0A1628', fontSize: '.6rem', fontWeight: 700, padding: '.18rem .5rem', borderRadius: '5px', fontFamily: "'Plus Jakarta Sans', sans-serif" },
  info: { position: 'absolute', bottom: '0.85rem', left: '0.85rem', right: '0.85rem' },
  name: { fontFamily: "'Outfit', sans-serif", fontWeight: 800, fontSize: '.88rem', color: '#fff', lineHeight: 1.2, marginBottom: '.2rem' },
  meta: { fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: '.68rem', color: 'rgba(255,255,255,0.65)' },
  playingDot: { position: 'absolute', top: '.75rem', right: '.75rem', display: 'flex', alignItems: 'center', gap: '.3rem', background: 'rgba(192,38,30,0.85)', borderRadius: '99px', padding: '.2rem .5rem' },
}
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#F4F5F7', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <section style={ps.section}>
        <SectionHeader tag="Student Testimonials" title="Placement Stories" sub="Tap any card to play — hear directly from students who got placed" icon={<MdVideoLibrary size={16} color="#C0261E" />} />
        <div style={ps.reelOuter} className="reel-scroll">
          <div style={ps.reelInner}>
            {LOCAL_VIDEOS.map(v => (
              <div key={v.id} style={ps.reelItem}><LocalVideoCard video={v} /></div>
            ))}
          </div>
        </div>
      ...
      <section style={{ ...ps.section, background: '#0A1628', maxWidth: '100%', padding: 'clamp(2rem,4vw,3rem) clamp(1rem,3vw,2rem)' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          <SectionHeader tag="Shorts & Reels" title="YouTube Shorts" sub="Quick insights, success moments and placement tips in short format" icon={<RiYoutubeLine size={16} color="#D4AF37" />} light />
          <div style={ps.shortsGrid}>
            {SHORT_EMBEDS.map((s, i) => (
              <div key={i} style={ps.shortCard} className="short-card">
                <div style={ps.shortEmbed}>
                  <iframe src={`https://www.youtube.com/embed/${s.id}?rel=0`} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', border: 'none' }} allowFullScreen title={s.label} loading="lazy" />
                </div>
          ...
          <p style={{ ...ps.scrollHint, color: 'rgba(255,255,255,0.3)', textAlign: 'center', marginTop: '1.25rem' }}>Instagram Reels integration requires embed approval from Meta</p>
        </div>
      </section>

      {/* ── SECTION 3 — LONG YOUTUBE VIDEOS (16:9) ── */}
      <section style={ps.section}>
        <SectionHeader tag="Full Sessions" title="YouTube Videos" sub="In-depth placement drives, mock interviews, and preparation sessions" icon={<MdOutlineOndemandVideo size={16} color="#C0261E" />} />
        <div style={ps.longGrid} className="long-grid">
          {LONG_VIDEOS.map((v, i) => (
            <div key={i} style={ps.longCard} className="long-card">
              <div style={ps.longEmbed}>
                <iframe src={`https://www.youtube.com/embed/${v.embedId}?rel=0`} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', border: 'none', borderRadius: '10px 10px 0 0' }} allowFullScreen title={v.name} loading="lazy" />
              </div>
...
const ps = {
  ...
  reelItem: { width: '200px', flexShrink: 0 },
  scrollHint: { fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: '.7rem', color: '#A0AEC0', textAlign: 'center', marginTop: '.6rem' },
  shortsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '1rem' },
  shortCard: { background: '#fff', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.08)', transition: 'transform .2s, box-shadow .2s', cursor: 'pointer' },
  shortEmbed: { position: 'relative', aspectRatio: '9/16', background: '#0A1628' },
  shortLabel: { display: 'flex', alignItems: 'center', gap: '.35rem', padding: '.55rem .65rem', borderTop: '1px solid #EEF1F8' },
  longGrid: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.25rem' },
  ...
  longTitle: { fontFamily: "'Outfit',sans-serif", fontWeight: 700, fontSize: '.82rem', color: '#0A1628', lineHeight: 1.4, marginBottom: '.45rem' },
  longMeta: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  longViews: { fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: '.68rem', color: '#8896A8' },
  longDur: { fontFamily: "'Outfit',sans-serif", fontWeight: 700, fontSize: '.68rem', color: '#fff', background: '#C0261E', padding: '.1rem .4rem', borderRadius: '4px' },
}